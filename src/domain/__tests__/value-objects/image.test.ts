...
import { Url } from '../../value-objects/url'; // Url もインポートして、Urlインスタンスが作られるか確認
...
describe('Image Value Object', () => {
  ...
  describe('Image.isValid static method', () => {
    ...
    test('should return false for invalid image data (basic validation)', () => {
    ...
    test('should return false for invalid image formats (.gif, .webp, no extension)', () => {
      ...
      expect(Image.isValid({ url: 'https://valid.com/img?query=test.png', altText: 'Valid Alt' })).toBe(false); // クエリパラメータがあるURLで拡張子を検出できない場合
    });
  });
});